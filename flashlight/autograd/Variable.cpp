...
Variable Variable::as(af::dtype newType) {
  ...
  auto gradFunc = [](std::vector<Variable>& inputs,
    ...
    af::array gradVal;
    if (grad_output.type() != input.type()) {
      gradVal = grad_output.array().as(input.type());
    } else {
      gradVal = grad_output.array();
    }
    input.addGrad(Variable(gradVal, false));
  };
...
Variable Variable::withoutData() const {
  ...
  other.array() = this->array().as(this->type());
  return other;
}
