...
constexpr unsigned int kDefaultStreamFlags = hipStreamNonBlocking;
...
class NcclContext {
 public:
  ...
  hipStream_t getReductionStream() const;
  hipStream_t getWorkerStream() const;
 ...
 private:
  ...
  hipStream_t reductionStream_;
  // CUDA stream in which hipMemcpyAsync calls run if in contiguous mode
  hipStream_t workerStream_;
...
void allReduceMultiple(
  ...
  hipStream_t workerStream = ncclContext.getWorkerStream();
...
hipStream_t NcclContext::getReductionStream() const {
  return reductionStream_;
}

hipStream_t NcclContext::getWorkerStream() const {
  return workerStream_;
}

hipEvent_t NcclContext::getEvent() const {
  return event_;
}
...
void* NcclContext::getCoalesceBuffer() {
  std::call_once(allocBuffer_, [&]() {
    FL_CUDA_CHECK(
        hipMalloc(&coalesceBuffer_, DistributedConstants::kCoalesceCacheSize));
  });
...
void NcclContext::createCudaResources() {
  ...
  FL_CUDA_CHECK(hipStreamCreateWithFlags(
  ...
  FL_CUDA_CHECK(
      hipStreamCreateWithFlags(&workerStream_, detail::kDefaultStreamFlags));

  FL_CUDA_CHECK(hipEventCreate(&event_, cuda::detail::kCudaEventDefaultFlags));
}
...
// DEBUG : ncclCommDestroy disabled as it leads to segfault.
  ...
  FL_CUDA_CHECK(hipEventDestroy(event_));
#endif
...
// destroy the stream by default.
  ...
  FL_CUDA_CHECK(hipStreamDestroy(reductionStream_));
  FL_CUDA_CHECK(hipStreamDestroy(workerStream_));
#endif
...
// default, as driver shutdown will clean up this memory anyways.
  ...
  if (coalesceBuffer_ != nullptr) {
    FL_CUDA_CHECK(hipFree(coalesceBuffer_));
  }
#endif
