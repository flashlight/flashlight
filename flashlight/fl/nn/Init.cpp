...
Tensor uniform(Shape shape, double min, double max, fl::dtype type) {
  Tensor result = fl::rand(shape, type);
  result = (max - min) * result + min;
  return result;
}
Tensor normal(Shape shape, double stdv, double mean, fl::dtype type) {
...
Tensor
kaimingUniform(Shape shape, int fanIn, fl::dtype type /* = fl::dtype::f32 */) {
...
Tensor
kaimingNormal(Shape shape, int fanIn, fl::dtype type /* = fl::dtype::f32 */) {
  double stdv = std::sqrt(1.0 / static_cast<double>(fanIn));
...
Tensor erfinv(const Tensor& y) {
  ...
  dem = (d[1] * z + d[0]) * z + 1.0;
  // TODO{fl::Tensor}{operator} - af::sign - zero case?
...
Variable constant(double val, Shape dims, fl::dtype type, bool calcGrad) {
...
Variable identity(Shape dims, fl::dtype type, bool calcGrad) {
...
Variable
uniform(Shape dims, double min, double max, fl::dtype type, bool calcGrad) {
...
Variable
normal(Shape dims, double stdv, double mean, fl::dtype type, bool calcGrad) {
