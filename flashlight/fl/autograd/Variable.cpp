...
Variable::Variable(af::array data, bool calcGrad) {
  // numdims is 4 since we can't know actual dims. This ctor will be removed
  // once array literals are removed.
  sharedData_->data =
      toTensor<ArrayFireTensor>(std::move(data), /* numDims = */ 4);
...
af::array& Variable::array() const {
  return toArray(sharedData_->data);
}
