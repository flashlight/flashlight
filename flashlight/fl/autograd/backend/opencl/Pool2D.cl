 ...
 * - output which is expected to be of dimensions [ox, oy, chan, batch].
 * - index which is expected to be of dimensions [ox * oy, chan, batch].
 * index output is the input to max_pool_bwd().
 * Supports sx>=px and sy>=py. Also assumes that px<=wx and py<=wy.
 */
void __kernel max_pool_fwd(
  ...
  const int xStart = (winX == 0) ? px : 0;
  const int yStart = (winY == 0) ? py : 0;
  const int xEnd = (winX == (ox - 1)) ? wx - px : wx;
  const int yEnd = (winY == (oy - 1)) ? wy - py : wy;
...
/**
 * Implements the backward part of 2D max pooling over grad and output
 * with the same dimensions as the input argument to max_pool_fwd().
 * The index argument is the result of max_pool_fwd().
 * output is expected to be a tensor of all zeros.
 */
