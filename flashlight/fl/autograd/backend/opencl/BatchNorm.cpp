...
Variable batchnorm(
  ...
  if (train) {
    ...
    // // TODO: These values are not updated by CUDA in AutogradTests so we
    // //       are not updating them here either. Why ?    // runningMeanDims
    // = momentum * runningMeanDims + (1 - momentum) * sampleMean;
    // runningVarDims = momentum * runningVarDims + (1 - momentum) *
    // sampleVar; runningMean = fl::moddims(runningMeanDims,
    // runningMean.dims()); runningVar = fl::moddims(runningVarDims,
    // runningVar.dims());
  } else {
