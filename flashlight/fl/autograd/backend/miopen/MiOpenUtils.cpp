...
struct Handle {
  ...
  Handle() : handle(nullptr) {
    cl_command_queue oclQ = fl::ocl::getQueue();
    MIOPEN_CHECK_ERR(miopenCreateWithStream(
        &handle, static_cast<miopenAcceleratorQueue_t>(oclQ)));
  }
  ~Handle() {
...
void createAndSetTensorDescriptor(
...
TensorDescriptor::TensorDescriptor(const af::array& input) {
  createAndSetTensorDescriptor(input, descriptor);
}
