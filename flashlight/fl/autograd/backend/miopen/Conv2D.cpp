...
namespace immediate {
/**
 * MIOpen v2.0 introduces the immediate which removes the requirement for the
 * miopenFindConvolution*() calls and their associated runtime costs. The
 * sequence of operations for in immediate mode is similar to launching regular
 * convolutions in MIOpen i.e. through the use of the miopenFindConvolution*()
 * API. However, in this case the different APIs have much lower runtime cost.
 * See:
...
Variable conv2d(
  ...
  miopenHandle_t handle = getMiOpenHandle();
  {
    ...
    // The simplest workaround for zero size workspace is to set it to a minimal
    // size.  Zero size workSpaceSize size is invalid input to DevicePtr.
  ...
  auto gradFunc = [sx, sy, px, py, dx, dy, hasBias, groups](
    ...
    auto convolutionBackwardData = [&hndl, &in, oneg, zerog, dx, dy, &wt](
      ...
      if (in.isCalcGrad()) {
        ...
        MIOPEN_CHECK_ERR(miopenConvolutionBackwardDataGetWorkSpaceSize(
            ...
            iDesc.descriptor,
            &workSpaceSize));
        // The simplest workaround for zero size workspace is to set it to a
        // minimal size.  Zero size workSpaceSize size is invalid input to
        // DevicePtr.
        workSpaceSize = std::max(workSpaceSize, sizeof(float));
    ...
    auto convolutionBackwardFilter = [&hndl, &wt, oneg, zerog](
      ...
      if (wt.isCalcGrad()) {
        ...
        // DevicePtr.
        auto worksspaceSize =
            std::max(bwdFilterAlgoBestPerf.workspace_size, sizeof(float));
        af::array ws = af::array(worksspaceSize);
