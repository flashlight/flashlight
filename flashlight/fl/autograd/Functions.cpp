...
Variable moddims(const Variable& input, const Shape& dims) {
  ...
  for (int i = 0; i < dims.ndim(); ++i) {
    if (inferDims[i] == 0 && i + 1 > input.numdims()) {
      throw std::invalid_argument(
...
Variable weightedCategoricalCrossEntropy(
  ...
  // target -- [X1, X2, X3]
if (input.numdims() < targets.numdims() - 1) {
  throw std::invalid_argument("weightedCategoricalCrossEntropy: input must have one more than the ""number of target dimensions minus 1");
}
  ...
  if (fl::any((targets.tensor() < 0) || (targets.tensor() >= C))
          .scalar<char>()) {
    throw std::invalid_argument(
        "weightedCategoricalCrossEntropy: target contains elements out of valid range ""[0, num_categories) in categorical cross entropy");
  }
