...
void getBatchnormMetadata(
  ...
  if (minAxis == 0) {
    ...
    inDescDimsOut = Shape(
        {1, 1, nfeatures, static_cast<long long>(input.size() / nfeatures)});
    ...
    inDescDimsOut = Shape(
        {1,
         static_cast<long long>(input.size() / (nfeatures * batchsz)),
         nfeatures,
         batchsz});
    wtDescDimsOut = Shape({1, 1, nfeatures});
  }
...
Tensor CudnnAutogradExtension::batchnorm(
  ...
  if (!weight.isEmpty() && weight.size() != wtDescDims.elements()) {
    throw std::invalid_argument("[BatchNorm] Invalid shape for weight.");
  }

  if (!bias.isEmpty() && bias.size() != wtDescDims.elements()) {
