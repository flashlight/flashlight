...
/**
 * A view into an underlying dataset with an offset and optional bounded size.
 *
 * The size of the `SpanDataset` can be explicitly specified but defaults to the
 ...
 * Example:
  ...
  \endcode
 */
class SpanDataset : public Dataset {
 public:
  /**
   ...
   * underlying dataset.
   * @param[in] size The size of the new dataset (if -1, uses the difference
   * between the input dataset size and the offset)
   */
