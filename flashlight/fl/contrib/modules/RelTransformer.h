 ...
 */
class RelTransformer : public Container {
 public:
  RelTransformer(
      int32_t modelDim,
      int32_t headDim,
 ...
 private:
  ...
  double pDropout_;
  double pLayerdrop_;
  bool preLN_, augment_;
  std::shared_ptr<Linear> w1_, w2_, wq_, wk_, wv_, wf_, rk_;
  std::shared_ptr<LayerNorm> norm1_, norm2_;
  fl::Variable rWbias_, rRbias_;
...
CEREAL_REGISTER_TYPE(fl::RelTransformer)