...
def create_training_list(img_folder, ann_file, output_file, low, high):
    coco = COCO(ann_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output_dir', help='Output dir .lst file', default='/private/home/padentomasello/data/coco-binsearch/between')
    parser.add_argument('--low', type=int, default=0)
    parser.add_argument('--high', type=int, default=10000000000000000000000000000000)
    ...
    output_dir = args.output_dir + f'-{args.low}-{args.high}';
    os.mkdir(output_dir)
    for (split, (img_folder, ann_file)) in paths.items():
        img_folder = os.path.join(root, img_folder)
        ann_file = os.path.join(root, 'annotations', ann_file)
        # output_file = os.path.join(args.output_dir, split + f'{[low-high]}' + '.lst')
        output_dir = args.output_dir + f'-{args.low}-{args.high}';
        output_file = os.path.join(output_dir, split + '.lst')
        create_training_list(img_folder, ann_file, output_file, args.low, args.high)
