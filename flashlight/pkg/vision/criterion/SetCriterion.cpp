...
Tensor span(const Shape& inDims, const int index) {
  Shape dims(std::vector<Dim>(
      std::max(inDims.ndim(), static_cast<size_t>(index + 1)), 1));
...
Tensor index(const Tensor& in, const std::vector<Tensor>& idxs) {
  ...
  output.flat(fl::range(linearIndices.size())) = in.flatten()(linearIndices);
  return output;
}
  ...
  for (auto idx : indices) {
    ...
    target_classes_full(srcIdxs, i) =
        fl::reshape(
            ...
            {static_cast<long long>(srcIdxs.size()), 1})
