...
Tensor index(const Tensor& in, const std::vector<Tensor>& idxs) {
  ...
  output.flat(fl::range(linearIndices.size())) = in.flat(linearIndices);
  return output;
}
...
SetCriterion::LossDict SetCriterion::forward(
  ...
  for (int i = 0; i < predBoxesAux.dims(3); i++) {
    auto predBoxes =
        predBoxesAux(fl::span, fl::span, fl::span, i);
    auto predLogits =
...
SetCriterion::LossDict SetCriterion::lossLabels(
  ...
  for (auto idx : indices) {
    ...
    target_classes_full(srcIdxs, i) = fl::reshape(targetClasses[i].tensor()(targetIdxs), {static_cast<long long>(srcIdxs.size()), 1}).astype(target_classes_full.type());
    i += 1;
  }
  ...
  auto lossCe = weightedCategoricalCrossEntropy(
      softmaxed, fl::Variable(target_classes_full.astype(fl::dtype::f32), false), weightVar, -1);
