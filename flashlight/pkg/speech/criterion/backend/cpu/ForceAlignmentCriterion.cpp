...
static void backward(
  ...
  Tensor inputGrad(N, T, B, inputGradVec.data());
  Tensor transGrad(N, N, transGradVec.data());
...
Variable ForceAlignmentCriterion::forward(
  ...
  ctx->targetVec = targetVar.toHostVector<int>();
  ctx->targetSizeVec = targetSize.toHostVector<int>();
  auto transVec = transVar.toHostVector<float>();
...
Tensor ForceAlignmentCriterion::viterbiPath(
    const Tensor& inputVar,
    const Tensor& targetVar) {
  const Tensor& transVar = param(0).array();
  ...
  const Tensor targetSize = getTargetSizeArray(targetVar, T);
  std::shared_ptr<Context> ctx = std::make_shared<Context>();
  std::vector<float> inputVec = inputVar.toHostVector<float>();
  ctx->targetVec = targetVar.toHostVector<int>();
  ctx->targetSizeVec = targetSize.toHostVector<int>();
  std::vector<float> transVec = transVar.toHostVector<float>();
  std::vector<float> lossVec(B);
  ctx->workspaceVec.assign(FAC::getWorkspaceSize(B, T, N, L), 0);
  ...
  FAC::viterbi(
      ...
      bestPaths.data(),
      ctx->workspaceVec.data());
  return Tensor(T, B, bestPaths.data());
}
} // namespace speech
} // namespace pkg
} // namespace fl
