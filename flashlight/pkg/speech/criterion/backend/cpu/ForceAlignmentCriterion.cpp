...
Variable ForceAlignmentCriterion::forward(
  ...
  ctx->targetVec = fl::afToVector<int>(targetVar);
  ctx->targetSizeVec = fl::afToVector<int>(targetSize);
  auto transVec = fl::afToVector<float>(transVar);
...
af::array ForceAlignmentCriterion::viterbiPath(
  ...
  std::vector<float> inputVec = fl::afToVector<float>(inputVar);
  ctx->targetVec = fl::afToVector<int>(targetVar);
  ctx->targetSizeVec = fl::afToVector<int>(targetSize);
  std::vector<float> transVec = fl::afToVector<float>(transVar);
  std::vector<float> lossVec(B);
  ctx->workspaceVec.assign(FAC::getWorkspaceSize(B, T, N, L), 0);
